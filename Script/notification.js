const $ = new Env('bark通知测试')

$.isNode() && require('dotenv').config()

const Enable_Bark_Notify = $.toObj($.isNode() ? process.env.ENABLE_BARK_NOTIFY : $.getval('enable_bark_notify'))

const Bark_Key = $.isNode() ? process.env.BARK_PUSH : $.getval('bark_key')

async function Bark_Nofify(title, subtitle, content, rawOpts = {}) {
    if (!Enable_Bark_Notify) return $.log(`配置中不进行Bark通知`)
...
!(async () => {
    await Bark_Nofify('标题', '副标题', '内容', {
        // $open: 'https://www.baidu.com/',
        $copy: '要复制的内容',
        $media: 'https://www.baidu.com/img/flexible/logo/pc/result@2.png'
    })
})()
    .catch((e) => $.logErr(e))
    .finally(() => $.done())
