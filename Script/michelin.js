 ...
 * 重写：打开微信小程序,点击探索+获取
 * [Script]
 * cron "0 15 13 * * 1" script-path=michelin.js, timeout=300, tag=米其林会员俱乐部
 * http-request ^https?:\/\/ulp\.michelin\.com\.cn\/op\/.+ script-path=michelin.cookie.js, timeout=10, tag=米其林俱乐部token
 *
 * [Mitm]
...
async function getCookie(){
    ...
    if($request&&$request.method!="OPTIONS"&&$request.url.match(/op\/activities/)){
        $.setdata(cookie, 'michelin_data');
        $.msg($.name,'🟢 获取会话成功', '');
    }
}
